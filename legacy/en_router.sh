...
ensure_dnsmasq_full() {
  ...
  if opkg install dnsmasq-full >/dev/null 2>&1; then
    say "✓ dnsmasq-full installed"
    return 0
  fi

  if [ "$had_dnsmasq" -eq 1 ]; then
    warn "Direct dnsmasq-full install failed; retrying with rollback protection"
    opkg remove dnsmasq >/dev/null 2>&1 || {
      err "Failed to remove dnsmasq before installing dnsmasq-full"
      exit 1
    }
  fi
  ...
  if [ "$had_dnsmasq" -eq 1 ] && ! package_installed dnsmasq; then
    opkg install dnsmasq >/dev/null 2>&1 || {
      err "Failed to restore dnsmasq after dnsmasq-full installation error"
      exit 1
    }
    warn "dnsmasq restored after dnsmasq-full installation failure"
  fi

  package_installed dnsmasq-full && return 0
  err "dnsmasq-full is unavailable; continuing with regular dnsmasq"
  return 1
}
...
if [ -z "${PUB_DEV:-}" ]; then
  err "Could not detect public interface (default route/WAN)."
  exit 1
fi
...
opkg update || {
  err "opkg update failed. Check internet/time/CA and rerun."
  exit 1
}
install_pkg() {
  local p="$1"
...
# dnsmasq-full (with DNS rollback protection)
ensure_dnsmasq_full
...
if [ -z "${ROOT_PW:-}" ]; then
  TTY=/dev/tty
  if [ -r "$TTY" ]; then
    printf "Enter root password (leave empty to keep current or auto-generate if missing): " >"$TTY"
    stty -echo <"$TTY" 2>/dev/null || true
    IFS= read -r ROOT_PW <"$TTY" || ROOT_PW=""
    stty echo <"$TTY" 2>/dev/null || true
    printf "\n" >"$TTY"
  else
    warn "TTY not available. Keeping current password or generating one if root has none."
    ROOT_PW=""
  fi
fi
root_has_password() {
...
if [ -n "${ROOT_PW:-}" ]; then
  printf "%s\n%s\n" "$ROOT_PW" "$ROOT_PW" | passwd root >/dev/null 2>&1 && {
    say "✓ Root password updated";
  } || {
    err "Failed to set user-defined password"
    exit 1
  }
else
  if root_has_password; then
    say "✓ Root password already set — leaving unchanged"
  else
    DEFAULT_PW="$(generate_random_secret)"
    [ -n "$DEFAULT_PW" ] || {
      err "Failed to generate a root password"
      exit 1
    }
    printf "%s\n%s\n" "$DEFAULT_PW" "$DEFAULT_PW" | passwd root >/dev/null 2>&1 && {
      say "✓ Root password generated automatically";
...
uclient-fetch -q -T 15 -O /etc/opkg/keys/ipk.pub "$PASSWALL_KEY_URL" \
 ...
 || warn "Could not download ipk.pub (non-critical)"
...
say "Passwall feeds target: $PW_RELEASE/$PW_ARCH"
...
if opkg update >/dev/null 2>&1; then
  ...
  warn "Failed to refresh Passwall feeds; skipping optional Passwall installation"
fi

# --------- OpenVPN server (tun0) ----------
say "=== Configuring OpenVPN server (tun0) ==="
ask_valid "OpenVPN port (UDP)" OPORT "1194" is_valid_port "Enter a numeric port between 1 and 65535"
ask_var "Client name (ovpn file)" CLIENT "client1"
CLIENT="$(sanitize_client_name "$CLIENT")"
[ -n "$CLIENT" ] || CLIENT="client1"
ask_valid "VPN IPv4 subnet"     VPN4_NET "10.99.0.0/24" is_valid_ipv4_cidr "Enter a valid IPv4 CIDR such as 10.99.0.0/24"
ask_valid "VPN IPv6 subnet"     VPN6_NET "fd42:4242:4242:1::/64" is_valid_ipv6_cidr "Enter a valid IPv6 CIDR such as fd42:4242:4242:1::/64"
...
[ -f "$OVPN_PKI/tc.key" ] || { openvpn --genkey secret "$OVPN_PKI/tc.key" 2>/dev/null && say "✓ TLS-crypt key generated"; }
...
# keep fw4 enabled for compatibility with dual-vpn-switcher/OpenVPN rules
...
say "Removing static web publication of the client config..."
cleanup_legacy_web_publish
/etc/init.d/uhttpd restart >/dev/null 2>&1 || true
say "✓ Web publication disabled; config stays in /root only"
...
if [ -f "/root/${CLIENT}.ovpn" ]; then
  say "✓ OVPN file saved at /root/${CLIENT}.ovpn"
else
  warn "✗ OVPN file was not created"
fi
...
echo "OpenVPN config on router: /root/${CLIENT}.ovpn"
echo "SCP download example: scp root@${SCP_TARGET}:/root/${CLIENT}.ovpn ."
echo "OpenVPN port: $OPORT (UDP)"
echo "LuCI password: hidden for security"
if [ -f /root/roadwarrior-credentials.txt ]; then
  echo "Generated root credentials: /root/roadwarrior-credentials.txt"
fi
echo ""
...
echo "3. Static web delivery of .ovpn is disabled; transfer the client file over SSH/SCP or SFTP."
echo "4. Quick rescue: /usr/sbin/rw-fix (does not change LAN, fixes DNS and default route)."
echo ""
say "Fetch the config with SCP: scp root@${SCP_TARGET}:/root/${CLIENT}.ovpn ."
say "Use account root and the password configured earlier"
